...
def avalia_gates_golden(
    ...
    return [
        ...
        _gate_fp(amostras),
        gate_velocidade(latencia_media_min, baseline, origem=f"golden set real ({len(amostras)} alertas)"),
        _gate_tokens(amostras),
        gate_latencia(p95_ms, origem=f"golden set real, {len(amostras)} alertas (não amostra sintética)"),
