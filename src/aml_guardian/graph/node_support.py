...
def handle_failure(
    state: InvestigationState, node: Node, attempt: int, reason: str, **extra: object
) -> dict[str, object]:
    ...
    try:
        ...
        pass
    record_node_event(
        state.alert_id, node, attempt, f"{node.value.upper()}_FAILED", state.state, AlertState.NEEDS_HUMAN
    )
    return {
